...
const defaultForm = { studentId: '', studentName: '', summary: '', outcome: '', date: '' };
...
export function ConsultationsModule({ consultations, setConsultations, globalSearch, showToast, addActivity }: Props) {
  ...
  const visible = useMemo(
    () => consultations.filter((c) => [c.id, c.studentId, c.studentName, c.summary, c.outcome, c.date].join(' ').toLowerCase().includes(query)),
    [consultations, query],
  );
    ...
    <div className="space-y-5 max-w-screen-xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-0.5" style={{ fontSize: 13 }}>Record consultation summaries and AI-assisted recommendations</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700">
                {['ID', 'Student', 'Date', 'Summary', 'Outcome', 'Actions'].map((h) => (
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {visible.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-12 text-slate-400" style={{ fontSize: 13 }}>No consultations logged</td></tr>
              ) : (
                visible.map((c) => (
      ...
      <Modal isOpen={!!viewConsult} title="Consultation Details" onClose={() => setViewConsult(null)}>
        {viewConsult && (
          <div className="space-y-3">
            {[['ID', viewConsult.id.slice(-6)], ['Student ID', viewConsult.studentId], ['Name', viewConsult.studentName], ['Date', viewConsult.date]].map(([k, v]) => (
